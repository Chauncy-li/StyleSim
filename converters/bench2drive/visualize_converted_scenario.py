...
class ScenarioVisualizer:
    ...
    def load_scenario_info(self):
        ...
        for filename, summary in list(self.dataset_summary.items())[:3]:
            ...
            number_summary = summary.get("number_summary", {})
            logger.info(f"  - Objects: {number_summary.get('num_objects', 'unknown')}")
            if "num_moving_objects" in number_summary:
                logger.info(f"  - Moving objects: {number_summary['num_moving_objects']}")
            logger.info(f"  - Track length: {summary.get('track_length', summary.get('length', 'unknown'))}")
            logger.info(f"  - SDC ID: {summary.get('sdc_id', 'unknown')}")
